...
for train_index, test_index in CV.split(X,y):
    ...
    if k == K-1:
        ...
        loglog(lambdas,train_err_vs_lambda.T,'b.-',lambdas,test_err_vs_lambda.T,'r.-',lambdas,np.full(len(lambdas),Error_test_nofeatures[k]),'g.-')
        xlabel('Regularization factor')
        ylabel('Squared error (crossvalidation)')
        legend(['Train error','Validation error', "Baseline error"])
        grid()
...
print(pd.DataFrame(data=np.concatenate((opt_lambdas, Error_test, Error_test_nofeatures), axis=1), columns=["Optimal lambda", "Test error" ,"Baseline test error"]))
